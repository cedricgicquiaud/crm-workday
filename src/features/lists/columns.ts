 ...
 */
import { CREATED_AT, isBaseColumn, UPDATED_AT } from "@/features/lists/sort";
...
/**
 * Colonnes visibles par défaut après la colonne titre : celles que la liste déclare, puis « Modifiée
 * le » — sauf quand la liste cite elle-même une colonne de base (D26) : elle a dit où les veut.
 */
export function defaultColumnKeys(list: string): string[] {
  const declared = getList(list).columns ?? [];
  return declared.some(isBaseColumn) ? [...declared] : [...declared, UPDATED_AT];
}
